...
if __name__ == "__main__":
    ...
    # Judge each of the 10 human-labeled questions: model_answer (A) vs ground_truth (B).
    # "A" or "tie" wins → judge thinks the model answer is correct/complete (label=1).
    # "B" wins (ground truth clearly better) → judge thinks the model answer is bad (label=0).
    with open(HUMAN_LABELS_PATH, encoding="utf-8") as f:
        human_data = json.load(f)
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        test_set_by_id = {item["id"]: item for item in json.load(f)}

    human_labels = [item["human_label"] for item in human_data]
    print(f"\nJudging {len(human_data)} answers against human labels...")

    kappa_judge_results: list[JudgeResult] = []
    judge_labels: list[int] = []
    for item in human_data:
        gt = test_set_by_id[item["question_id"]]["ground_truth"]
        r = swap_and_average(item["question"], item["model_answer"], gt)
        kappa_judge_results.append(r)
        label = 1 if r.final_winner in ("A", "tie") else 0
        judge_labels.append(label)
        match = "match" if label == item["human_label"] else "MISMATCH"
        print(f"  Q{item['question_id']}: judge={label} human={item['human_label']} ({match})")

    kappa = cohen_kappa(judge_labels, human_labels)
    print(f"\nCohen's κ: {kappa:.3f}")

    # --- Extra pairwise pairs (answer vs ground_truth) for a broader bias sample ---
    all_judge_results = list(kappa_judge_results)
    if os.path.exists(ANSWERS_PATH):
        with open(ANSWERS_PATH, encoding="utf-8") as f:
            answers = json.load(f)
        sample = answers[:5]
        print(f"\nRunning swap-and-average on {len(sample)} additional pairs (bias sample)...")
        for a in sample:
            r = swap_and_average(a["question"], a["answer"], a["ground_truth"])
            all_judge_results.append(r)

    # --- Bias report ---
    bias = bias_report(all_judge_results)
    print(f"\nBias report: {bias}")

    save_phase_b_report(all_judge_results, judge_labels, human_labels, kappa, bias)
